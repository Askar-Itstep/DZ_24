...
public class MainActivity extends AppCompatActivity {
    ...
    private String[] words =  {
            "Apple", "Lemon", "Hello", "World", "Board", "House", "Horse", "April", "Woman", "Table",
 "River", "Dream", "Green", "Stone", "Water",   "Rover»", "Width", "Drive", "Pause"  };
     ...
     *  будут располагаться буквы загаданного слова  * (Идентификаторы в макете : tv1, tv2, tv3, tv4, tv5)  */
    private  TextView[] tvSymbols =  new TextView[MainActivity.SYMBOLS_IN_WORD];

    /**  * Массив числовых идентификаторов текстовых полей  * TextView,в которых располагаются буквы загаданного  * слова.
    ...
    //поля для сохранен. состояния экрана
...
//            this.word = savedInstanceState.getString(MainActivity.BUNDLE_KEYWORD);
...
//нажатие на кнопку (какую из? см. стр. 115)
    public void btnClick(View v) {
        ...
        //--Идентифицируем кнопку------------------------
         if (v.getId() == R.id.btnSymbol)  {
             ...
             S = S.substring(0, 1); //если введено слово вместо буквы Слон С, Ульяна У
             if (this.word.toLowerCase().contains(S))   {
                 ...
                 for (int i = 0; i < this.tvSymbols.length; i++)    {
                     ...
                     if (symbol.toLowerCase().equals(S))     {//далее можно было replace??
...
//
//        outState.putInt(MainActivity.BUNDLE_KEY_COUNT, this.count);
//        outState.putString(MainActivity.BUNDLE_KEYWORD, this.word);
//        outState.putString(MainActivity.BUNDLE_KEY_CURWORD, this.curWord);
